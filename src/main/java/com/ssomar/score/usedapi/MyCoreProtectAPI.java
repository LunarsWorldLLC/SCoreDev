...
public class MyCoreProtectAPI {
     ...
     * Uses in-memory tracking instead of CoreProtect to avoid main thread blocking.
     *
     * Note: Only tracks blocks placed since server start. Blocks placed before
     * the plugin loaded will be considered natural.
     */
    public static boolean isNaturalBlock(Block block) {
        return PlayerPlacedBlockTracker.getInstance().isNatural(block);
    }
